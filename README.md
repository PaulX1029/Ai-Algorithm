...
- **README_RESNET.md**：包含 ResNet 算法的说明和实现细节。
...
- **v5**：包含 YOLOv5 算法的实现。具体内容如下：
  - **v5_Custom**：包含自定义的 YOLOv5 算法实现。
  - **v5_Original**：包含原始的 YOLOv5 算法实现（源码禁止改动，并附上版本及原始github带分支及tag的链接）。
- **v8**：包含 YOLOv8 算法的实现。具体内容如下：
  - **v8-Custom**：包含自定义的 YOLOv8 算法实现。
    - **custom_codes**：包含自定义的 YOLOv8 代码。
    - **train_cfgs**：包含自定义的 YOLOv8 训练配置文件。
    - **v8_scripts**：包含自定义的 YOLOv8 脚本。
